...
REGEXP="^(?!.*(fx|/ta/|/co2/)).*(tas/|tos|hfds|thetao).*gr.*$"
# have to be super careful when crunching input files as the duplicate grids can cause
# things to explode
REGEXP_WRANGLE_IN_FILES=".*tas/.*gr/.*"

netcdf-scm-crunch "${SRC_DIR}" "${CRUNCH_DIR}" "${CONTACT}"  --drs "${DRS}" --regexp "${REGEXP}" --small-number-workers 10 --medium-number-workers 2
