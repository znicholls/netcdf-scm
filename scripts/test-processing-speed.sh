...
DRS="CMIP6Output"
#REGEXP="^(?!.*(fx|ta/)).*(tas/|fco2nat|rsut|rlut|rsdt).*$"
REGEXP="^(?!.*(fx|/ta/|/co2/)).*Amon.*$"

exec 3>&1 4>&2
SRC_DIR_TIME=$( { time netcdf-scm-crunch "${SRC_DIR}" "${CRUNCH_DIR}" "${CONTACT}"  --drs "${DRS}" --regexp "${REGEXP}" --force --small-number-workers 10 1>&3 2>&4; } 2>&1 )
exec 3>&- 4>&-
...
exec 3>&1 4>&2
SRC_DIR_BIG_TIME=$( { time netcdf-scm-crunch "${SRC_DIR_BIG}" "${CRUNCH_DIR}" "${CONTACT}"  --drs "${DRS}" --regexp "${REGEXP}" --force --medium-number-workers 4 1>&3 2>&4; } 2>&1 )
exec 3>&- 4>&-
