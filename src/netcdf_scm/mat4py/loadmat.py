...
def unpack(endian, fmt, data):
    ...
    if fmt == "s":
        ...
        val = struct.unpack("".join([endian, str(len(data)), "s"]), data)[0]
    else:
...
def read_element_tag(fd, endian):
    ...
    if num_bytes > 0:
        ...
        mtpn = mtpn & 0xFFFF
        if num_bytes > 4:
            raise ParseError("Error parsing Small Data Element (SDE) " "formatted data")
...
def read_elements(fd, endian, mtps, is_name=False):
    ...
    if mtps and mtpn not in [etypes[mtp]["n"] for mtp in mtps]:
        raise ParseError(
            "Got type {}, expected {}".format(
                mtpn,
                " / ".join("{} ({})".format(etypes[mtp]["n"], mtp) for mtp in mtps),
            )
        )
    if not data:
    ...
    if is_name:
        ...
        fmt = "s"
        val = [unpack(endian, fmt, s) for s in data.split(b"\0") if s]
        if len(val) == 0:
            val = ""
        elif len(val) == 1:
            val = asstr(val[0])
...
def read_numeric_array(fd, endian, header, data_etypes):
    ...
    array = [
        list(data[c * rowcount + r] for c in range(colcount)) for r in range(rowcount)
    ]
...
def loadmat(filename, meta=False):
    ...
    while not eof(fd):
        ...
        name = hdr["name"]
        if name in mdict:
            raise ParseError('Duplicate variable name "{}" in mat file.'.format(name))
